...
struct WebService{
            ...
            .responseJSON { response in
                ...
                case .success(let value):
                    if let responseValue = value as? [String: AnyObject] {
                        if let message = responseValue[WSResponseParams.WS_RESP_PARAM_MESSAGE] as? String, message == AlertStrings.SUCCESSFULLY_UPLOAD_PROFILE_PIC {
    ...
    static func uploadImageWithURL(api:Api, dictImage : [String:UIImage], parameter : [String : Any], header:Bool = false, completion:@escaping(CompletionBlock)){
        ...
        _ = (try? URLRequest(url: urlString, method: .post, headers: headers))!

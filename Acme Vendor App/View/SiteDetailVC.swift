...
class SiteDetailVC: UIViewController {
    ...
    func setValues() {
        ...
        } else if Cookies.userInfo()?.type == "City Head" {
            btnReject.isHidden = siteDetail?.cityHeadStatus == "Approved"
            btnApprove.isHidden = siteDetail?.cityHeadStatus == "Rejected"
            siteDetail?.cityHeadStatus == "Approved" ? btnApprove.setTitle("Approved", for: .normal) : btnApprove.setTitle("Approve", for: .normal)
            btnApprove.isUserInteractionEnabled = siteDetail?.cityHeadStatus != "Approved"
            
            siteDetail?.cityHeadStatus == "Rejected" ? btnReject.setTitle("Rejected", for: .normal) : btnReject.setTitle("Reject", for: .normal)
            btnReject.isUserInteractionEnabled = siteDetail?.cityHeadStatus != "Rejected"
        }
        cnstHeightBottomStack.constant = Cookies.userInfo()?.type != "rhm" && Cookies.userInfo()?.type != "vendor" && Cookies.userInfo()?.type != "City Head" ? 0 : 55
        vwStackView.isHidden = Cookies.userInfo()?.type != "rhm" && Cookies.userInfo()?.type != "vendor" && Cookies.userInfo()?.type != "City Head"
                ...
                if (self.locationManager.authorizationStatus == .authorizedWhenInUse ||
                   self.locationManager.authorizationStatus == .authorizedAlways) {
                    ...
                    if let coordinates = self.convertStringToCLLocationDegrees(latitudeString: self.siteDetail?.latitude ?? "", longitudeString: self.siteDetail?.longitude ?? "") {
                        ...
                        let givenLatLong = CLLocationCoordinate2DMake(coordinates.0, coordinates.1)
                        let personLatLong = CLLocationCoordinate2D(latitude: self.currentLoc.coordinate.latitude, longitude: self.currentLoc.coordinate.longitude)
